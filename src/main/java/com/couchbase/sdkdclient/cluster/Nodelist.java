...
import com.couchbase.sdkdclient.cluster.NodeHost.State;
...
public final class Nodelist {
    ...
    public Collection<NodeHost> getUpgrade() {
        ...
        for (NodeHost nn:allNodes) {
            if (nn.getState().containsAll(EnumSet.of(State.UPGRADE, State.ACTIVE))) {
                ll.add(nn);
            }
        }
        return ll;
    ...
    void activate(Collection<NodeHost> nodes) {
        for (NodeHost nn : nodes) {
            activeNodes.add(nn);
            if (nn.getState().contains(State.UPGRADE)) {
                nn.setState(EnumSet.of(State.UPGRADE, State.ACTIVE));
            } else {
                nn.setState(EnumSet.of(State.ACTIVE));
            }
        }
    }
    ...
    void remove(Collection<NodeHost> nodes) {
        ...
        for (NodeHost nn : nodes) {
            if (nn.getState().contains(State.UPGRADE)) {
                nn.setState(EnumSet.of(State.UPGRADE, State.FREE));
            } else {
                nn.setState(EnumSet.of(State.FREE));
            }
        }
    }
    ...
    void fail(Collection<NodeHost> nodes) {
        ...
        for (NodeHost nn : nodes) {
            if (nn.getState().contains(State.UPGRADE)) {
                nn.setState(EnumSet.of(State.UPGRADE, State.FAILED_OVER));
            } else {
                nn.setState(EnumSet.of(State.FAILED_OVER));
            }
        }
    }
