...
import com.couchbase.sdkdclient.ssh.SSHCapturedCommand;
...
public class RemoteCommands {
  ...
  public static OSInfo getSystemInfo(SSHConnection conn) throws IOException {
    ...
    if (out.contains(ClusterInstaller.ARCH_64)) {
      _arch = ClusterInstaller.ARCH_64;
    } else if (out.contains(ClusterInstaller.AMD_64)){
      _arch = ClusterInstaller.AMD_64;
    } else if (out.contains(ClusterInstaller.ARCH_32)) {
      _arch = ClusterInstaller.ARCH_32;
    }
    ...
    if (os.compareToIgnoreCase("ubuntu") == 0 ) {
      ...
      _packageType = "deb";
      if (_arch == ClusterInstaller.ARCH_64) {
        _arch = ClusterInstaller.AMD_64;
      }
    }
