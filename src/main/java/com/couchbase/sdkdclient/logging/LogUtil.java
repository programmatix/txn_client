...
public class LogUtil {
  ...
  public static void addDebugOption(OptionParser parser) {
    if (debugOption.isSealed()) {
      return;
    }

    OptionTree optTree = new OptionTreeBuilder()
            .description("Debugging and logging options")
            .group("debug")
            .build();
    optTree.addOption(debugOption);
    new InfoOption("debug-help", "Show debug levels and loggers", parser) {
      @Override public Action call() {
        System.out.print(getLoggersHelp());
        return Action.EXIT;
      }
    };

    new InfoOption("disable-colors", "Don't show colors", parser) {
      @Override public Action call() {
        LoggingOptions.setColorsEnabled(false);
        return Action.CONTINUE;
      }
    };

    parser.addTarget(optTree);
  }
